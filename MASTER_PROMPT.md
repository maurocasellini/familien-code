...
### Geburtszeit mit Punkt still verworfen → Mond und Aszendent falsch (04.09.2026)
**Problem:** Das Geburtszeit-Feld ist ein freies Textfeld. Susana tippte «05.40» (Punkt statt Doppelpunkt). `pages/api/astrology.js` akzeptierte nur `/^\d{1,2}:\d{2}/`, hat die Zeit deshalb kommentarlos verworfen und ist auf den Default `hour = 12` (Mittag) zurückgefallen, mit `timeKnown = false`. Folge im Fall Brigitte (16.07.1978, 05:40, Altstätten): Mond stand auf Schütze 1°54 statt korrekt Skorpion 28°07 (der Mond läuft ca. 0.6°/h, in den 6.3 Stunden Differenz lief er über die Zeichengrenze), und der Aszendent wurde gar nicht berechnet (Häuser nur bei `timeKnown && coords`). Alle langsamen Planeten blieben unauffällig korrekt, deshalb fiel nur der Mond auf. Der Fehler zog sich durch den ganzen Report, weil das Modell den Mittags-Mond kommentarlos als Tatsache deutete. `pages/api/humandesign.js` und `pages/api/connection.js` hatten denselben Bug in anderer Form (`birthTime.split(':')` → NaN).
**Lösung:**
1. Neues Modul `lib/timeparse.js` mit `normalizeTime()` / `normalizeTimeString()`. Akzeptiert 05:40, 5:40, 05.40, 5.40, 05,40, 0540, 540, 5h40, «05 40», «5:40 Uhr», «5:40 PM»; validiert 0–23 / 0–59; gibt immer kanonisches `HH:MM` zurück oder `null`.
2. Eingesetzt in `astrology.js`, `humandesign.js`, `connection.js` und in `getPerson()` in `pages/index.js` (damit auch die Grunddaten-Zeile kanonisch ist).
3. `astrology.js` liefert neu `timeUsed` und `timeRejected` und unterscheidet «keine Zeit angegeben» von «Zeit angegeben, aber unlesbar».
4. Zweite Absicherung im Prompt: fehlt eine gültige Zeit, wird die Mond-Zeile in `astroBlock()` explizit als UNSICHER markiert, mit der Anweisung, das Mondzeichen nicht als Tatsache zu deuten. Vorher war die Mittagsposition für das Modell nicht von einer echten unterscheidbar.
5. Live-Zeit-Prüfer unter dem Feld (analog zum Ort-Prüfer): grün «✓ gelesen als 05:40 Uhr» oder rot «✗ Zeit nicht lesbar». Rein clientseitig, Event-Delegation, greift auch bei den dynamischen Kinderfeldern.
**Lernerfahrung:** Ein stiller Fallback auf einen plausiblen Default ist bei Berechnungsdaten gefährlicher als ein harter Fehler. Wenn eine Eingabe verworfen wird, muss das sichtbar sein — im UI für Susana und im Prompt für das Modell.
